...
public class MarkServiceImpl implements MarkService {
  ...
  public void delete(Long markId) {
    log.debug("Request to delete MarkID {}", markId);
  ...
  public StatisticStudentMarkDTO getAllMarkOfStudent(Long studentId) {
    ...
    List<SimplifiedMarkDTO> studentAllMark = markDTOList.stream().map(MarkDTO::convertToSimplified).collect(Collectors.toList());
  ...
  public List<StatisticStudentMarkDTO> getAllMarkOfAllStudents() {
    ...
    Map<Long, Double> averageMarksMap = averageMarks
      .stream()
      .collect(Collectors.toMap(entry -> (Long) entry[0], entry -> (Double) entry[1]));

    Map<Long, List<Mark>> studentMarksMap = allMarkOfAllStudents
      .stream()
      .collect(
    ...
    for (Map.Entry<Long, List<Mark>> entry : studentMarksMap.entrySet()) {
      ...
      List<SimplifiedMarkDTO> simplifiedMarkDTOs = markDTOs.stream().map(MarkDTO::convertToSimplified).collect(Collectors.toList());
